...
> 基于 uni-app + Vue 3 的农业数据采集微信小程序，用于玉米表型采集。

## 项目简介

狮山云瞳是一款面向农业科研场景的微信小程序，主要服务于田间玉米种植研究中的**表型采集**核心业务。通过拍照采集作物影像，结合地理位置信息和计算机视觉技术，实现对玉米植株表型指标（株高、茎粗、叶夹角、穗位高等）数据记录与分析，目前已经实现本地采集功能，上传功能和检测植株表型还未集成
...
| **结果查看(未实现)** | 展示分析结果：表型指标（株高、茎粗等）和拷种指标（长度、粒数等） |
| **个人信息** | 查看账户信息、退出登录 |
